...
const UserLabelPage = ({ params }: { params: { awbNumber?: string } }) => {
  const [awbNumber, setAwbNumber] = useState(params.awbNumber?.toString() ?? "");
  ...
  useEffect(() => {
    const getOrderDetails = async () => {
      if (awbNumber !== "") {
        const currentOrderDetails = await mutateAsync({ awbNumber: awbNumber });
